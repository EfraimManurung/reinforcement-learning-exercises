...
class SimpleCorridor(gym.Env):
    ...
    Observations are floats indicating the current field index, e.g. 0.0 for starting position,
    1.0 for the field next to the starting position, etc.. Reward are -0.1 for all steps,
    except when reaching the goal (+1.0).
...
# Except to reach an optimal episode reward of '-0.1*18 + 1.0 = -0.8'.
for i in range(5):
    results = algo.train()
...
# Get the initial observation (should be: [0.0] for the staring position).
obs, info = env.reset()
